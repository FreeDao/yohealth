...
@EViewGroup(R.layout.view_adultbalancenofatdata)
public class View_AdultBalanceNoFatData extends LinearLayout {
	Context mContext;

	@ViewById
	TextView tvMsg, tvTop_1, tvTop_2, tvBottom_1, tvBottom_2, tvBottomRight,tvTopRightBottom,tvTopRightTop;
	@ViewById
	ImageView imgTopRightTop,imgTopRightBottom;
	@ViewById
	...
	String percent1="--",percent2="--",tizhongshuyu="--",jianyitizhong1="--",jianyitizhong2="--",zhifanghanliang="--",jianyizhifang1="--",jianyizhifang2="--",shuifenhanliang="--",jianyishuifen1="--",jianyishuifen2="--";
	PopupWindow rankPopupWindow,suggestPopupWindow;
	View rankPopupContentView,suggestPopupContentView, HideRankPopupContentView,HideSuggestPopupContentView;
	public View_AdultBalanceNoFatData(Context context) {
		super(context);
		mContext = context;
		// init();
	...
	public View_AdultBalanceNoFatData(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		// init();
		// TODO Auto-generated constructor stub
	...
	void showRank()
	{
		if (rankPopupWindow == null) {
			...
			TextView tvPre1 =(TextView)rankPopupContentView.findViewById(R.id.tvPre1);
			TextView tvPre2 =(TextView)rankPopupContentView.findViewById(R.id.tvPre2);
	...
	void showSuggest()
	{
		if (suggestPopupWindow == null) {
			LayoutInflater inflater = LayoutInflater.from(mContext);
			suggestPopupContentView = inflater.inflate(R.layout.pop_jianyi, null);
			TextView tvPianShou =(TextView)suggestPopupContentView.findViewById(R.id.tvPianShou);
			TextView tvJianYi1 =(TextView)suggestPopupContentView.findViewById(R.id.tvJianYi1);
			TextView tvJianYi2 =(TextView)suggestPopupContentView.findViewById(R.id.tvJianYi2);
			TextView tvZhiFan =(TextView)suggestPopupContentView.findViewById(R.id.tvZhiFan);
			TextView tvJianYiZhiFan1 =(TextView)suggestPopupContentView.findViewById(R.id.tvJianYiZhiFan1);
			TextView tvJianYiZhiFan2 =(TextView)suggestPopupContentView.findViewById(R.id.tvJianYiZhiFan2);
			TextView tvShuiFen =(TextView)suggestPopupContentView.findViewById(R.id.tvShuiFen);
			TextView tvJianYiShuiFen1 =(TextView)suggestPopupContentView.findViewById(R.id.tvJianYiShuiFen1);
			TextView tvJianYiShuiFen2 =(TextView)suggestPopupContentView.findViewById(R.id.tvJianYiShuiFen2);
		...
		if (isSuggestOpenPopWindow) {
			...
			anchorCenter[1] = location[1] ;
			suggestPopupWindow.showAtLocation(btnSuggest, Gravity.TOP | Gravity.LEFT,
					anchorCenter[0], anchorCenter[1] - hei-20);
	...
	public void setData(String weight, String val, String data) {
		...
		if ((devicestate & (int) Math.pow(2, 1)) > 0) {
			...
			BalanceDataMDL lastData=new BalanceDataDAL(mContext).SelectLastData(member.getMemberid(), member.getClientid());
			float thisVal=ObjectHelper.Convert2Float(weight);
			new getRanking2Task().execute(member.getUserid(),member.getMemberid(),bmi,member.getSex(),member.getHeight(),weight);
			DecimalFormat fnum = new DecimalFormat("##0.0"); 
			if(lastData!=null)
			{
				float lastVal=ObjectHelper.Convert2Float(lastData.getWeight());
				
				float cha=thisVal-lastVal;
				if(cha>0)
				{
					imgTopRightTop.setImageResource(R.drawable.ic_red_up);
					tvTopRightTop.setText(fnum.format(cha));
				}
				else {
					imgTopRightTop.setImageResource(R.drawable.ic_red_dowm);
					tvTopRightTop.setText(fnum.format((0-cha)));
				}
			}
			float targetVal=ObjectHelper.Convert2Float(member.getTargetweight());
			
			float cha=thisVal-targetVal;
			if(cha>0)
			{
				imgTopRightBottom.setImageResource(R.drawable.ic_red_up);
				tvTopRightBottom.setText(fnum.format(cha));
			}
			else {
				imgTopRightBottom.setImageResource(R.drawable.ic_red_dowm);
				tvTopRightBottom.setText(fnum.format((0-cha)));
			}
			float bmiVal=ObjectHelper.Convert2Float(bmi);
			if(bmiVal<18.5)
			{
				//UIHelper.SetText(R.id.tvBottomRight, LanguageConstants.alert_AccountFormatErr, this);
				tvBottomRight.setText("偏瘦");
			}
			else if(bmiVal>=18.5&& bmiVal<24)
			{
				tvBottomRight.setText("正常");
				//UIHelper.SetText(R.id.tvBottomRight, LanguageConstants.alert_AccountFormatErr, this);
			}
			else if(bmiVal>=25&& bmiVal<29.9)
			{
				tvBottomRight.setText("偏胖");
				//UIHelper.SetText(R.id.tvBottomRight, LanguageConstants.alert_AccountFormatErr, this);
			}
			else if(bmiVal>=29.9&& bmiVal<39.9)
			{
				tvBottomRight.setText("肥胖");
				//UIHelper.SetText(R.id.tvBottomRight, LanguageConstants.alert_AccountFormatErr, this);
			}
			else if(bmiVal>=39.9)
			{
				tvBottomRight.setText("严重肥胖");
				//UIHelper.SetText(R.id.tvBottomRight, LanguageConstants.alert_AccountFormatErr, this);
			}
			
			doSuggest(bmi,member.getHeight(),"","",member.getSex(),member.getBirthday());
...
//			balanceDataMDL.setWater(valArray[4]);
		...
		if (weight.contains(".")) {
			...
			tvTop_1.setText(strs[0]);
			tvTop_2.setText(strs[1].subSequence(0, 1));
		} else {
			tvTop_1.setText(weight);
			tvTop_2.setText("0");
		}
		if (bmi.contains(".")) {
			String[] strs = bmi.split("\\.");
			tvBottom_1.setText(strs[0]);
			tvBottom_2.setText(strs[1].subSequence(0, 1));
		} else {
			tvBottom_1.setText(bmi);
			tvBottom_2.setText("0");
		}
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			// TODO Auto-generated method stub
			if (!isLockData)
				tvMsg.setText(LanguageHelper.GetVal(LanguageConstants.lb_playViewLinkStateTip_WeightNotConfirm, mContext));
			if (isRed) {
	...
	public void doSuggest(String bmi,String heightval,String fatstring,String waterstring,String sex,String agestring)
	{
		float bmiVal=ObjectHelper.Convert2Float(bmi);
	...
	class getRanking2Task extends AsyncTask<String, Object, Object>{
		...
		protected Object doInBackground(String... params) {
			...
			JSONObject jsonObject = new BalanceDataWS(mContext).getRanking2(userid,memberid,bmi,sex,height,weight);
	...
	void init() {
		...
		HideSuggestPopupContentView = inflater.inflate(R.layout.pop_jianyi, null);
