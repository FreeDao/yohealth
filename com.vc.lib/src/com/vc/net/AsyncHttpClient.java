...
public class AsyncHttpClient
{
	...
	/** Future 以context为键 把连接线程保存起来 以便根据context取消*/
	...
	public AsyncHttpClient()
	{
		...
		HttpProtocolParams.setUserAgent(httpParams, String.format(
				"uroad-android-httpclient/%s (http://www.u-road.com/)", VERSION));
		...
		//针对在HTTPClient采用压缩格式的文件的传输，必须采用拦截器进行特殊的处理  
	 ...
	 * 给请求设置cookie,一般是PersistentCookieStore的一个实例
	 * @param cookieStore
	 ...
	 * 设置自定义的线程池,默认的线程池是 Executors.newCachedThreadPool()
	 * @param threadPool
	...
	/**
	 * Sets the connection time oout. By default, 30 seconds
	 * 设置连接超时时间,以毫秒为单位
	 * @param timeout
	 ...
	 * 如果有用context作为参数的请求,可以调用此方法取消请求
	 * @param context
	 ...
	 * 进行一次http get请求 用context作为参数可以 调用cancelRequests 取消请求
	 * @param context
